...
class ActionPrompt(PromptJson):
	...
	def json_obj(self)->dict:
		...
		wait = {'type': 'object', 'required': ['func', 'time_dur'],
				'properties': {'func': {'description': '车等待一段时间', 'const': 'wait'},
		...
		beep_x = {'type': 'object', 'required': ['func', 'time_dur'],
				  'properties': {'func': {'description': '蜂鸣器，需要发声，', 'const': 'beep_x'},
								 'frequency': {'description': '发声次数', 'type': 'number'}}
				  }
		...
		light_s = {'type': 'object', 'required': ['func', 'time_dur'],
				   'properties': {'func': {'description': '灯，需要闪烁', 'const': 'light_s'},
								  'frequency': {'description': '闪烁次数', 'type': 'number'}}
				   }

		schema_actions = {'type': 'array', 'required': ['items'],
						  'items': {'anyOf': [schema_move, schema_beep, schema_light, light_s, beep_x, wait],
									'minItems': 1
									}
						  }
...
class ErnieBotWrap():
	...
	def __init__(self):
		...
		erniebot.access_token = '5476bf0362e04efed5f66df1191b1b6635a0fdb3'
		self.msgs = []
