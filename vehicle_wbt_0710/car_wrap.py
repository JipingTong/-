...
class MyCar(MecanumBase):
    ...
    def do_action_list(self, acitons):
        def move_func(x, y, angle):
            self.set_pos_offset([x, y, angle*math.pi / 160])

        def wait(time_dur):
            self.delay(time_dur)

        def light_func(time_dur):
            for i in range(1, 5):
        ...
        def beep_func(time_dur):
            start_time = time.time()
            end_time = start_time + time_dur
            while time.time() < end_time:
                self.beep_ocr()

        def light_shanshuo(frequency):
            a = 1
            while a <= frequency:
                for i in range(1, 5):
                    self.light.set_light(i, 255, 255, 255)
        ...
        def beep_fashengcishu(frequency):
            a = 1
            while a <= frequency:
                self.beep()
                self.delay(0.5)
        ...
        action_map = {'move': move_func, 'light': light_func, 'beep': beep_func, 'light_s': light_shanshuo, 'beep_x': beep_fashengcishu,
                      'wait': wait}
    ...
    # 侧面摄像头进行位置定位
    def lane_det_location(self, speed, pt_tar=[0, 1, 'pedestrian',  0, -0.15, -0.48, 0.15, 0.52], dis_out=0.22, side=1, det='task'):
        ...
        find_tar = False
        #pt_tar = [0, 1, 'pedestrian', 0, -0.15, -0.48, 0.19, 0.68]
    ...
    def get_ocr(self, time_out=10):
        ...
        while True:
            ...
            text = self.ocr_rec(img[120:, 120:440])
            print(text)
            if text_count(text):
                return text
    ...
    def manage(self, programs_list:list, order_index=0):
        ...
        def xunhang():
            self.lane_dis_offset(0.3, 50)

        programs_suffix = [all_task, lane_test, xunhang, self.debug]
