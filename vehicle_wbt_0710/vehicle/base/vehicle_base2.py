...
class ArmBase():
    def __init__(self) -> None:
        ...
        while True:
            try:
                ...
                self.dipan = self.config["dipan"]
                self.xiabi = self.config["xiabi"]
                self.zhongbi = self.config["zhongbi"]
                self.shangbi = self.config["shangbi"]
                self.zhuazi = self.config["zhuazi"]
                ...
                # 默认设置
                self.config = {"pos_enable":True, "side":-1, "pos_vert":0, "pos_horiz":0, "angle_list":[90, 0, -100],
                               #"horiz_moto_param":{"port_id":5, "revese":-1}, "vert_moto_param":{"port_id":6, "revese":1},
                                "horiz_limit_port":3, "vert_limit_port":4,
                               "dipan":1, "xiabi":2, "zhongbi":3, "shangbi":4, "zhuazi":7,
                               "arm_servo_id":1,"vert_pid_param":{"Kp":20, "Ki":0.0, "Kd":0.4, "setpoint": 0}, "horiz_pid_param":{"Kp":20, "Ki":0.0, "Kd":0.4, "setpoint": 0},
                                "vert_threshold":[-0.23, 0.23], "horiz_threshold":[-0.23, 0.23]} #grap_servo_port原本为1
                self.pos_vert = 0
                self.pos_horiz = 0
                self.save_yaml()
                flag_reset = True
        ...
        self.key = Key4Btn(2)
        self.dipan = ServoPwm(self.dipan)
        self.xiabi = ServoPwm(self.xiabi)
        self.zhongbi = ServoPwm(self.zhongbi)
        self.shangbi = ServoPwm(self.shangbi)
        self.zhuazi = ServoPwm(self.zhuazi)
    ...
    def dipan_angle(self, angle):
        self.dipan.set(70, angle)

    def xiabi_angle(self, angle):
        self.xiabi.set(60, angle)

    def zhongbi_angle(self, angle):
        self.zhongbi.set(60, angle)

    def shangbi_angle(self, angle):
        self.shangbi.set(60, angle)

    def zhuazi_angle(self, angle):
        self.zhuazi.set(70, angle)
...
class MecanumBase():
    ...
    def beep(self):
        self.buzzer.set(100, 10)

    def beep_ocr(self):
        self.buzzer.set(100, 60)
