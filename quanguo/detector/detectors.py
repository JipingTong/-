...
class DetectionResult:
    ...
    def error_from_point(self, point):
        ...
        return error_x, error_y
    def error_updown(self,point):
        center_x = int((self.box[0] + self.box[2]) / 2)
        center_y = int((self.box[1] + self.box[3]) / 2)
        error_x = int(center_x - point[0])
        error_y = int(center_y - point[1])
        return error_x, error_y
...
class TaskDetector(Detector):
    ...
    def __init__(self, num):
        ...
        model_path = os.path.join(root_path, self.task_set_model(num))
        Detector.__init__(self, model_path)

    def task_set_model(self, num):
        if num == 1:
            model_task = "model/task_22_1"

        else:
            model_task = "model/task_22"

        print("the task_set_model: ", model_task)
...
class Cruiser:
    ...
    def __init__(self, num):
        ...
        # self.model_dir = None
        model_path = os.path.join(root_path, self.set_model(num))

        # model_dir = "model/cruise1"
    ...
    def set_model(self, num):
        
        if num == 1:
            model_dir = "model/cruise1"
        elif num == 2:
            model_dir = "model/cruise2"
        elif num == 3:
            model_dir = "model/cruise"

        elif num == 4:
            model_dir = "model/cruise4"
        else:
            model_dir = "model/cruise"

        print("the set_model: ", model_dir)
